;;; This file is distributed as part of the Staapl package (c) 2006
;;; Tom Schouten. It is an adaptation of gforth.el which contained the
;;; copyright notice below the line.

;;; Changes to the original gforth.el:
;;;
;;; * removed objects and oof words
;;; * staapl is always in compile mode (state == t) except for
;;;   [ .. ] when it is in CAT mode (state == nil)
;;; * file starts in compile mode
;;; * ';' does not terminate indentation, and ':' jumps to left margin


;; Workarounds for deep nesting for command completion.
...
TYPE should be one of 'definition-starter', 'definition-ender', 'compile-only',
...
;; definition-starter and definition-ender just change the value of
;; state.  in staapl, ':' and ';' are just immediate words (macros) and
;; do not change state. the only words that do are '[' and ']'

;; i'm commenting out all the things that staapl does not support.
...
;; 	((";") definition-ender (font-lock-keyword-face . 1))
	...
	(("dummy") compile-only (font-lock-keyword-face . 1))
...
;; 	 "[ \t\n]" t name (font-lock-function-name-face . 3))
	...
	(("if" "begin" "ahead" "do" "?do" "+do" "u+do" "-do" "u-do" "for" 
	  ...
	  "repeat" "again" "else" "while" "leave" "?leave"
	  "loop" "+loop" "-loop" "next" "endcase" "endof" "try" 
	  "route" "asm"
          "table" "_table"  ;; byte and bit lut, like route
...
;; 	 "[ \t\n]" t name (font-lock-variable-name-face . 3))
	...
	(("create" "variable" "constant" "2variable" "2constant" 
;;	  "fvariable" "fconstant" "value" "field" "user" "vocabulary" 
;;	  "create-interpret/compile"
	  "flag" "end")
	 immediate (font-lock-type-face . 2)
...
(setq forth-indent-words
      '((("if" "begin" "for" "route" "asm" "case" "of" 
;;	  "do" "?do" "+do" "-do" "u+do"
;;	  "u-do" "?dup-if" "?dup-0=-if" "try" 
...
;;	  ":noname" "code" "struct" "m:" ":m" "class" "interface"
	  ":")
	 (-100 . 2) (-2 . 2))  ;; hack to return to left margin
	(("variable" "flag" "constant" "2constant" 
	  "macro" "forth") 
	 (-2 . 1) (-2 . 0))
;	(("(") (0 . 2) (0 . 2))
;	((")") (0 . -2) (0 . -2))
...
(defun forth-customize-words ()
 ...
 to `forth-words' and `forth-indent-words', respective."
...
(defun forth-delete-properties (from to)
  (remove-text-properties 
   from to '(face nil fontified nil 
		  forth-parsed nil forth-word nil 
		  forth-state nil)))
...
(defun forth-set-word-properties (state data)
  (let* ((start (point))
    ...
    (when
	(or (not state) (eq type 'compile-only) (eq type 'immediate))
...
;;    (setq state (get-text-property (point) 'forth-state))
      (setq state
	    (if (= from 1) t ;; HACK: state starts t 
	      (get-text-property (point) 'forth-state)))
...
;(define-key forth-mode-map ":" 'forth-electric-colon)
;(define-key forth-mode-map "(" 'forth-electric-paren)

;(define-key forth-mode-map ":" 'self-insert-command)
;(define-key forth-mode-map "(" 'self-insert-command)
;(define-key forth-mode-map " " 'self-insert-command)

;(define-key forth-mode-map " " 'forth-electric-space)



;; Electric Keys
;; (defun forth-electric-colon ()
;;   (interactive) (insert ":") (forth-indent-command))

;; (defun forth-electric-paren ()
;;   (interactive) (insert "(") (forth-indent-command))

;; (defun forth-electric-space ()
;;   (interactive) (insert " ") (forth-indent-command))
...
FIXME: this is different for staapl.el
...
(when (memq 'comint features)
  ...
  (define-key forth-mode-map "\C-c\C-p" 'forth-send-paragraph)
...
;;;###autoload
  ...
  ;; FIXME
  (defun init-forth-staapl ()
    (interactive)
    "Piggyback on CAT."
    ; (staapl)
    (setq forth-program-name nil)
    (setq forth-process-buffer "*cat*"))
...
;;     (pop-to-buffer "*forth*"))
  ...
  (defun forth-send-region (start end)
    ...
    (comint-send-string (forth-proc) ") r\n"))
